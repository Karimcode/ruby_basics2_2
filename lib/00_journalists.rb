...
puts handle_twitter.length

# Quelle est le handle le plus court de cette liste ?

smallest_handle = handle_twitter.sort_by { |word| word.length } # classer par longueur,
puts smallest_handle[0] # sortir le premier résultat

# Combien y-a-t'il de handle contenant 5 caractères (le @ ne compte pas pour un caractère)
five_characters = handle_twitter.select { |word| word.size == 6 }
puts five_characters.length

# Combien commencent par une majuscule (première lettre juste après le @) ?

first_cap_word = handle_twitter.select { |w| w[1].match?(/[A-Z]/) } #
puts first_cap_word.length # sortir le premier résultat
...
result = handle_twitter.sort {|left, right| left.length <=> right.length}
puts result

# # Quelle est la position dans l'array de la personne @epenser ?
a = handle_twitter.index('@epenser')
puts a
...
result = []
a = handle_twitter.length
b = 1
c = []
while b != 0 do
    for i in 1..a do
      result =  handle_twitter.select { |word| word.size == i }
      # puts  result.length
      b = result.length
      c[i] = result.length
    end
